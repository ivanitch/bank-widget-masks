...
def test_get_mask_card_number_invalid_value(invalid_card_numbers):
    for card_num in invalid_card_numbers:
        with pytest.raises(ValueError):
            get_mask_card_number(card_num)


def test_get_mask_account(card_accounts):
    for card_num, expected in card_accounts:
        assert get_mask_account(card_num) == expected


def test_get_mask_account_invalid_value(invalid_card_accounts):
